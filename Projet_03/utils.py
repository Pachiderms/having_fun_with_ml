...
def _draw_france(ax):
    """
    Description:
        Draw the outline of metropolitan France as a soft background on an axis.
...
def plot_stages(villages, labels, title, numbered=False):
    ...
    Arguments:
        villages: DataFrame with latitude and longitude columns
        labels: array-like of cluster or stage labels, one per village
        title: title of the figure
        numbered: whether to write the stage number at the center of each stage
...
def plot_final_tour(villages, stage_paths, stage_distances):
    """
    Description:
        Draw the complete Tour with every stage path in its own color.

    Arguments:
        villages: DataFrame with latitude and longitude columns
        stage_paths: dictionary mapping each stage number to its village indices
            in visiting order
        stage_distances: dictionary mapping each stage number to its length in
            kilometers
...
def plot_stage_zoom(villages, path, stage_number, distance):
    """
    Description:
        Draw a single stage in detail, with village names and start/finish markers.

    Arguments:
        villages: DataFrame with village, latitude and longitude columns
        path: list of village indices, in visiting order
        stage_number: number of the stage, used in the title
        distance: length of the stage in kilometers, used in the title
